...
Until a build step is added, keep these in sync manually. The PTX currently implements the first conservative `k = 1` scoring path: one CUDA thread per request, a full scan over all agents, a `dimensions == 16` unrolled distance path with a generic fallback, precomputed per-agent score weights from the CPU-equivalent score coefficients, best-available output, base/effective distance output, and an ideal-unavailable flag.
...
The runtime boundary now has RAII wrappers for the retained primary context, streams, CUDA events, typed device buffers, modules, function handles, and a q-tom-specific route-kernel launch wrapper. `RouteAgentsKernelArgs` consumes a validated `CudaBufferPlan`, checks buffer lengths, keeps output buffers mutably borrowed, packs the exact `qtom_route_agents_k1` parameters, launches synchronously, and reports typed CUDA errors. The `k = 1` execution helper allocates, precomputes per-agent score weights, copies, launches, synchronizes, copies back, and decodes CUDA output arrays into CPU-shaped `RoutingResult` values.

The public boundary is now intentionally narrow: `CudaRouter::route_batch` may return CUDA results only when `cuda-runtime` is enabled, the driver initializes, request/state shapes validate, and every request uses `k = 1`. The benchmark CLI exposes this as `--write-cuda-golden <path>` for the default `k = 1` fixture, `--cuda-parity <path>` for CPU-vs-public-CUDA parity, `--cuda-timing <path>` for whole-call timing after parity is checked, and `--cuda-scale` for generated `k = 1`, `dims = 16` scale probes across multiple agent counts. CUDA parity keeps task IDs, candidate IDs, flags, and candidate counts exact, while allowing a small absolute tolerance on floating score fields. CUDA timing now reports runtime init, host preparation, allocation, host/device copies, module/stream setup, host launch/sync wall time, CUDA event device time, inferred host overhead, and decode, and includes a `cuda-reuse` row that reuses runtime/module/buffer resources for fixed-shape batches. Decode now avoids per-result linear agent-id lookup by building an agent-id index once per batch. Event timing on the RTX 4060 shows launch/sync overhead is small compared with device work; the first kernel-body pass reduced the reusable `k = 1` fixture from roughly 5.9 ms to roughly 5.0 ms while preserving parity. `--cuda-scale` shows exact scoring remains roughly linear in candidate count, so memory curation or lossy prefilter layers should focus on handing CUDA compact candidate sets for exact scoring. Do not add `k > 1` until the `k = 1` public path remains boringly deterministic through the first kernel-body improvements. That work must follow `docs/cuda-safety.md`: checked sizes, explicit ownership, small documented `unsafe` blocks, and typed errors for every CUDA failure.
