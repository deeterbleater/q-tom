...
| Governance safety | Partial. | `TopologyGovernanceStore` reads proposal, snapshot, and rollback JSONL ledgers together; validates unknown proposal, unknown snapshot, and invalid rollback route-decision fixture references with `InvalidRollbackRouteDecisionRef`; resolves `active_topology_snapshot`; applies rollback through `apply_rollback`; records governance visibility with `TopologyShadowed`, `TopologyCanaried`, and `TopologyRolledBack`; and exposes a topology governance projection from proposal, shadow, canary, commit, and rollback events. | Add end-to-end shadow and canary route-decision fixtures before promotion. |
